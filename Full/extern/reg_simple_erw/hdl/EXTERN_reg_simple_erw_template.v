...
#(
    ...
    parameter INPUT_WIDTH = 2*REG_WIDTH+OP_WIDTH+1,
...
(
    ...
    wire    [REG_WIDTH-1:0]        compVal_in           = tuple_in_@EXTERN_NAME@_input_DATA[INPUT_WIDTH-2               -: REG_WIDTH];
    wire    [REG_WIDTH-1:0]        newVal_in            = tuple_in_@EXTERN_NAME@_input_DATA[INPUT_WIDTH-REG_WIDTH-2     -: REG_WIDTH];
    wire    [OP_WIDTH-1:0]         opCode_in            = tuple_in_@EXTERN_NAME@_input_DATA[INPUT_WIDTH-2*REG_WIDTH-2   -: OP_WIDTH];
    ...
    always @ (*) begin
        ...
        if (valid_in) begin
            // Check if compVal_in matches currentVal_r and eventually overwrite currentVal_r_next
            match_r_next = (compVal_in == currentVal_r) ? 1'b1 : 1'b0;
            currentVal_r_next = ( (opCode_in == WRITE_OP) && (match_r_next == 1) ) ? newVal_in : currentVal_r;
        end
    end
    ...
    assign tuple_out_@EXTERN_NAME@_output_DATA  = match_r;
